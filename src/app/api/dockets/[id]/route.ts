...
export async function PATCH(
  ...
  try {
    ...
    const before: Snapshot = {
      ...
      fuel_charge: dec(existing.fuelCharge),
      clearing_charge: dec(existing.clearingCharge),
      air_service_charge: dec(existing.airServiceCharge),
    ...
    const after: Snapshot = {
      ...
      fuel_charge: Number(body.fuel_charge || 0),
      clearing_charge: Number(body.clearing_charge || 0),
      air_service_charge: Number(body.air_service_charge || 0),
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.$executeRaw`
        UPDATE "cargo_dockets" SET
          ...
          fuel_charge = ${paiseToDecimalString(Math.round(Number(body.fuel_charge || 0) * 100))}::decimal,
          clearing_charge = ${paiseToDecimalString(Math.round(Number(body.clearing_charge || 0) * 100))}::decimal,
          air_service_charge = ${paiseToDecimalString(Math.round(Number(body.air_service_charge || 0) * 100))}::decimal,
